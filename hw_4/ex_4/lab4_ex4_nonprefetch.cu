...
int main(int argc, char **argv) {
    ...
    cusparseDnVecDescr_t vecDescrOut;
...
/*
    ...
    cusparseCheck(cusparseCreateDnVec(&vecDescr, dimX, temp, CUDA_R_64F));
    cusparseCheck(cusparseCreateDnVec(&vecDescrOut, dimX, tmp, CUDA_R_64F));
    ...
    cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, spMatDescr, vecDescr, &zero, vecDescrOut, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, &bufferSize));
    ...
    for (int it = 0; it < nsteps; ++it) {
        ...
        //@@ tmp = 1 * A * temp + 0 * tmp
        cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, spMatDescr, vecDescr, &zero, vecDescrOut, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, buffer));
    ...
    cusparseCheck(cusparseDestroyDnVec(vecDescr));
    cusparseCheck(cusparseDestroyDnVec(vecDescrOut));
